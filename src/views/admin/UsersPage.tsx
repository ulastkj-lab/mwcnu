...
export default function UsersPage() {
  ...
  const handleOpenEdit = (user: UserItem) => {
    ...
    setFormData({
      ...
      password: user.password || 'mwcnukarpaw',
      role: user.role,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name.trim() || !formData.username.trim() || !formData.role) {
      setAlert({ type: 'error', message: 'Nama, User (Username), dan Role/Kredensial wajib diisi.' });
      return;
    }
    ...
    const payload = {
      ...
      password: formData.password.trim() || 'mwcnukarpaw',
      role: formData.role,
  ...
  const filteredUsers = users.filter(u => {
    ...
    const matchesSearch = nameLower.includes(query) || uname.includes(query);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 font-medium mt-1">
            Daftarkan, sunting, dan atur kredensial username & password pengguna SIM MWC NU sesuai tingkatan struktural masing-masing.
          </p>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/75 border-b border-slate-150 text-[10px] uppercase font-bold text-slate-400 tracking-wider">
                  ...
                  <th className="py-3 px-4">Kredensial (Role)</th>
                ...
                {filteredUsers.length > 0 ? (
                  filteredUsers.map((u) => (
                    <tr key={u.id} className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="py-3.5 px-4">
                        <div className="flex items-center gap-1.5 font-mono text-xs font-semibold text-slate-700 bg-slate-100 px-2.5 py-1 rounded-lg w-max border border-slate-200">
                      ...
                      <td className="py-3.5 px-4">
                        <div className="flex items-center gap-1.5 font-mono text-xs text-slate-600">
                          ...
                          <span>{showPasswordMap[u.id] ? (u.password || 'mwcnukarpaw') : '••••••••'}</span>
                          <button 
                            type="button"
                            onClick={() => setShowPasswordMap(prev => ({ ...prev, [u.id]: !prev[u.id] }))}
                            className="text-slate-400 hover:text-slate-600 p-0.5 rounded cursor-pointer"
                            title={showPasswordMap[u.id] ? "Sembunyikan Password" : "Tampilkan Password"}
                          >
        ...
        <div className="fixed inset-0 bg-slate-900/60 z-50 flex items-center justify-center p-4 backdrop-blur-xs">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden transform transition-all border border-slate-200 flex flex-col max-h-[90vh]">
            ...
            <div className="p-4 bg-gradient-to-r from-emerald-900 to-brand-emerald-dark text-white flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-extrabold text-sm">
                  {modalMode === 'create' ? 'Daftarkan Pengguna Baru' : 'Sunting User & Kredensial Pengguna'}
                </h3>
              </div>
              <button 
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4 overflow-y-auto flex-1 text-left">
              ...
              <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-3 flex gap-2 text-[10px] text-emerald-800 font-medium leading-relaxed">
                <Info className="w-4 h-4 text-brand-emerald shrink-0 mt-0.5" />
                <span>
                  Atur nama pengurus, username akun, kata sandi (password), serta kredensial/role hak akses sistem SIM MWC NU.
                </span>
              </div>

              {/* Name Field */}
              <div className="space-y-1">
                <label className="text-[11px] font-bold text-slate-500 uppercase block">Nama Lengkap Pengurus</label>
                <input
                  type="text"
                  required
              ...
              {/* Username Field */}
              <div className="space-y-1">
                <label className="text-[11px] font-bold text-slate-500 uppercase block">User (Username Login)</label>
                <div className="relative">
                  <User className="w-4 h-4 text-slate-400 absolute left-3 top-2.5" />
                  <input
                    type="text"
                    required
                    placeholder="Contoh: fauzi_mwc"
                    className="w-full bg-white text-slate-800 pl-9 pr-3 py-2 border border-slate-200 rounded-xl text-xs font-mono font-semibold focus:outline-none focus:ring-1 focus:ring-brand-emerald"
              ...
              {/* Password Field */}
              <div className="space-y-1">
                <label className="text-[11px] font-bold text-slate-500 uppercase block">Password (Kata Sandi)</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-white text-slate-800 pl-9 pr-9 py-2 border border-slate-200 rounded-xl text-xs font-mono font-semibold focus:outline-none focus:ring-1 focus:ring-brand-emerald"
              ...
              {/* Role dropdown */}
              <div className="space-y-1">
                <label className="text-[11px] font-bold text-slate-500 uppercase block">Kredensial / Level Hak Akses (Role)</label>
                <select
                  required
